...
const storage = multer.diskStorage({
  destination: function (req, file, cb) {
    // Usa o caminho absoluto para garantir que a pasta 'uploads' esteja no local correto
    cb(null, path.join(__dirname, 'uploads')); // Caminho absoluto para a pasta 'uploads'
  },
  filename: function (req, file, cb) {
    // Renomeia o arquivo com o timestamp atual e o nome original
...
app.post('/usuario/inserir', upload.single('foto'), async (req, res) => {
  try {
    ...
    const novoUsuario = await Usuario.create({
      ...
      foto: req.file.path // Armazena o caminho da imagem
    });
